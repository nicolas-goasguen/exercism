...
// next must be in l and not nil, newNode must not be nil
func (l *List) insertBefore(newNode *Node, next *Node) {
	if next == nil || newNode == nil {
		panic("next or newNode values must not be nil")
	}
	if next == l.first {
		l.first = newNode
	}
	linkNodes(newNode, next)
}

// prev must be in l and not nil
func (l *List) insertAfter(newNode *Node, prev *Node) {
	if prev == nil || newNode == nil {
		panic("prev or newNode values must not be nil")
	}
	if prev == l.Last() {
		l.last = newNode
	}
	linkNodes(prev, newNode)
}
