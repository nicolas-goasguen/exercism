...
func parseQuarterDelivery(meetingDT time.Time, delivery string) (string, bool) {
	...
	if err != nil || deliveryQ > 4 || deliveryQ < 0 {
		return "", false
	}
	var deliveryDT time.Time
	if int(meetingDT.Month()) < deliveryQ*3+1 {
		deliveryDT = time.Date(meetingDT.Year(), time.Month(deliveryQ*3+1), 0, 0, 0, 0, 0, meetingDT.Location())
	} else {
		deliveryDT = time.Date(meetingDT.Year()+1, time.Month(deliveryQ*3+1), 0, 0, 0, 0, 0, meetingDT.Location())
	}
	offset := negativeWorkdayOffset(deliveryDT.Weekday())
	return time.Date(deliveryDT.Year(), deliveryDT.Month(), deliveryDT.Day()-offset, 8, 0, 0, 0, meetingDT.Location()).Format(fmtDT), true
}
...
func DeliveryDate(start, delivery string) string {
	...
	parsed := ""
	parsed, ok := parseFixedDelivery(meetingDT, delivery)
	if ok {
		return parsed
	}
	parsed, ok = parseMonthDelivery(meetingDT, delivery)
	if ok {
		return parsed
	}
	parsed, ok = parseQuarterDelivery(meetingDT, delivery)
	if ok {
		return parsed
	}
	return parsed
